...
* [`src/clj/reframe_basic/`](src/clj/reframe_basic/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/reframe_basic/`](src/cljs/reframe_basic/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/reframe_basic/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/reframe_basic/`](test/cljs/reframe_basic/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `reframe-basic` project root directory
...
in the [`src/clj/reframe_basic/`](src/clj/reframe_basic/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/reframe_basic/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns reframe-basic.example
  (:require [reframe-basic.config :as config])
